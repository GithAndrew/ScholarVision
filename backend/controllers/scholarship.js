...
exports.addScholarship = async (req,res) => {
    ...
    if(token.user.role == 'admin' || token.user.role == 'member' || token.user.role == 'donor'){
        ...
        try {
            ...
            await UserLog.create(token.user, 'create', `scholarship ${scholarship._id}`)
...
exports.editScholarship = async (req,res) => {
    ...
    if(token.user.role == 'admin' || token.user.role == 'member'){
        ...
        try{
            ...
            await UserLog.create(token.user, 'edit', `scholarship ${edit._id}`)
    ...
    if(token.user.role == 'admin' || token.user.role == 'member'){
        ...
        try{
            for(let i = 0; i < reqLength; i++){
                ...
                try{
                    scholarship = await Scholarship.getOne({_id: idList[i]});  //call to handler here
                    //console.log(scholarship);
                    if(scholarship){
                        await Delete.create("scholarship", scholarship);
                        await UserLog.create(token.user, 'delete', `scholarship ${scholarship._id}`)
...
exports.findAll = async (req, res) => {
    ...
    if(token.user.role == 'admin' || token.user.role == 'member' || token.user.role == 'scholar' || token.user.role == 'donor'){
            // sort = 1 if ascending, -1 if descending, 0 if no sort
