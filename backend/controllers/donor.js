...
const Donor = require('../handlers/donor');
...
exports.addDonor = async (req,res) => {
    ...
    const newDonor = {
        ...
        middle_name: body.middle_name,          //null if no suffix
        birthday: body.birthday,
        birthplace: body.birthplace,
        suffix: body.suffix,                    //null if no suffix
    ...
    try {
        ...
        await UserLog.create(token.user, 'create', `donor ${donor._id}`)
...
exports.editDonor = async (req,res) => {
    ...
    if(token.user.role == 'admin' || token.user.role == 'member'){
        ...
        try{
            ...
            await UserLog.create(token.user, 'edit', `donor ${edit._id}`)
    ...
    if(token.user.role == 'admin' || token.user.role == 'member'){
        ...
        try{
            for(let i = 0; i < reqLength; i++){
                ...
                let donor = null;
                try{
                    donor = await Donor.getOne({_id: idList[i]});  //call to handler here
                    //console.log(donor);
                    if(donor){
                        await Delete.create("donor", donor);
                        //console.log(deleteEntry)
                        await UserLog.create(token.user, 'delete', `donor ${donor._id}`)
